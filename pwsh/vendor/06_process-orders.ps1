# process orders
# when a accepted order got received, we need to process it
...
Register-EngineEvent -SourceIdentifier "OrderAccepted" -Action {
    ...
    $endpoint = "https://api.omas.app/v1/$($fullfilment.name):process"
    ...
    New-Event -SourceIdentifier "OrderProcessed" -MessageData $fullfilment | Out-Null    
} | Out-Null
